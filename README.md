...
- [The cattle STR database (CSDB)](#the-cattle-str-database-csdb)
...
| **Ae** | Effective number of alleles (= 1 / Σpᵢ²; reported as 0 at monomorphic loci, consistently with the other parameters) |
...
`testdata/` holds a small, self-contained dataset so you can exercise Step 2
end to end without supplying your own data:

| File | Contents |
|---|---|
| `test.vcf` | 10 samples x 1000 STR loci, taken from the HipSTR output of the cattle whole-genome data used in the accompanying study (SRA accessions SRR12452185-SRR12452194) |
| `test.config.DitoHex.bed` | The configuration rows for exactly those 1000 loci |
| `README.txt` | Provenance and the commands below |

The configuration is subset to match the VCF: `STR_analysis_pipeline.R` joins the
two on the locus ID, so every locus in the VCF needs a configuration entry.

Run it with filtering enabled (Mode T):

```bash
./pSTRminer poly \
  -o ./test_output \
  -C testdata/test.config.DitoHex.bed \
  -I testdata/test.vcf \
  -O smoke_test \
  -F T \
  --min-call-qual 0.9 \
...
Or skip filtering if you want to treat the input as already filtered (Mode F):

```bash
./pSTRminer poly -o ./test_output -C testdata/test.config.DitoHex.bed \
  -I testdata/test.vcf -O smoke_test -F F
```

Either run finishes in well under a minute and writes the files listed in
[Output files](#output-files) into `./test_output/`. As a quick check that the
run behaved, `smoke_test.forensic_parameters.txt` should carry the header

```
locus	N	Na	Ho	He	MP	PD	PE2	PE3	PIC	Ae
```

and, in Mode T, the log should report roughly 1200 calls dropped by the quality
filters.

The same dataset can be produced from any full-size HipSTR VCF; see
`testdata/README.txt` for the exact source files used here.

---

# The cattle STR database (CSDB)

`CSDB.txt` is the curated marker set reported in the accompanying study. It
lists the STR loci retained after applying the study's selection criteria to the
genome-wide screen: a genotyping success rate of at least 40% and a PIC of at
least 0.5 in the population concerned.

| Column | Description |
|---|---|
| `Chr`, `Start`, `End` | Locus coordinates on the bosTau9 reference |
| `PeriodSize` | Motif length in bp |
| `CopyNumbers` | Repeat number reported by TRF (non-integer for imperfect STRs) |
| `LocusID` | Identifier, shared with the pSTRminer configuration file |
| `MotifSequence` | Repeat motif |
| `Population` | Population(s) in which the locus met the criteria: `chn` (Chinese cattle), `afr` (African cattle), or `chn,afr` for both |
| `PIC_chn`, `Ae_chn`, `Na_chn`, `GR_chn` | Forensic parameters and genotyping success rate in Chinese cattle |
| `PIC_afr`, `Ae_afr`, `Na_afr`, `GR_afr` | The same in African cattle |

A locus that met the criteria in only one population still carries the other
population's values where they could be estimated, so the two can be compared
directly. `Ae` is 0 at monomorphic loci, consistently with the other parameters
(see [Forensic parameters](#forensic-parameters)).

`CSDB.txt` is a result of the accompanying study, not an output of the software.
To build an equivalent table for your own data, run Step 2 and filter
`<prefix>.forensic_parameters.txt` on the criteria that suit your application.
