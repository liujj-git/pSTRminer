...
# Calculate forensic parameters: N,Na,Ho,He,DP,MP,PE2,PE3,PIC
forensic_parameters_calculator <- function(x) {
  ...
  if (length(GT) == 0) {
    ...
    DP <- round(DP_value(allele_freq), 4)
...
# Calculate average DP and PIC
avg_DP <- mean(as.numeric(output1$DP[output1$DP != ""]), na.rm = TRUE)
...
writeLines(c(
  ...
  paste("  Average discrimination power (DP):", round(avg_DP, 4)),
