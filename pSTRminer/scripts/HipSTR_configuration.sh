...
THREADS_TRF=30
THREADS_PYTHON=40
# SCRIPT_DIR is the directory containing this script (and the bundled
# auxiliary scripts + TRF). Using BASH_SOURCE makes it independent of the
# current working directory, so output can be written to a separate location.
...
setup_software() {
    ...
    # Set up TRF in SCRIPT_DIR (where this script and bundled tools live).
    # Using an absolute target ($SCRIPT_DIR/...) keeps TRF discoverable by
    # later stages regardless of the current working directory.
    # Priority: (1) user-specified PSTRMINER_TRF env var,
    #           (2) trf409.legacylinux64 already in SCRIPT_DIR or PATH,
    #           (3) download from GitHub.
    if [[ -n "$PSTRMINER_TRF" ]] && [[ -f "$PSTRMINER_TRF" ]]; then
        echo "Using user-specified TRF: $PSTRMINER_TRF"
        ln -sf "$(cd "$(dirname "$PSTRMINER_TRF")" && pwd)/$(basename "$PSTRMINER_TRF")" "$SCRIPT_DIR/trf409.legacylinux64"
        chmod +x "$PSTRMINER_TRF" 2>/dev/null || true
    elif [[ -f "$SCRIPT_DIR/trf409.legacylinux64" ]]; then
        echo "TRF already available in script directory, skipping download..."
    elif command -v trf409.legacylinux64 &> /dev/null; then
        TRF_PATH=$(which trf409.legacylinux64)
        echo "Creating symbolic link to TRF from: $TRF_PATH"
        ln -sf "$TRF_PATH" "$SCRIPT_DIR/trf409.legacylinux64"
    else
        echo "Downloading Tandem Repeats Finder (TRF)..."
        wget -q https://github.com/Benson-Genomics-Lab/TRF/releases/download/v4.09.1/trf409.legacylinux64 -O "$SCRIPT_DIR/trf409.legacylinux64"
        chmod 755 "$SCRIPT_DIR/trf409.legacylinux64"
        echo "TRF downloaded"
    fi
    
    # HipSTR auxiliary scripts (run_TRF.sh, fix_trf_output.py, trf_parser.py,
    # analyze_overlaps.py) are bundled alongside this script in SCRIPT_DIR.
    # We only fall back to cloning HipSTR-references if they are NOT found,
    # which keeps the pipeline fully functional offline / when bundled.
    local bundled_ok=1
    for s in run_TRF.sh fix_trf_output.py trf_parser.py analyze_overlaps.py; do
    ...
    if [[ $bundled_ok -eq 1 ]]; then
        echo "Using bundled HipSTR auxiliary scripts from: $SCRIPT_DIR"
        # Create a HipSTR-references/scripts/ layout pointing to the bundled
        # scripts so downstream path references remain valid.
        mkdir -p "$SCRIPT_DIR/HipSTR-references/scripts"
        for s in run_TRF.sh fix_trf_output.py trf_parser.py analyze_overlaps.py; do
            ln -sf "$SCRIPT_DIR/$s" "$SCRIPT_DIR/HipSTR-references/scripts/$s"
        done
        chmod -R 755 "$SCRIPT_DIR/HipSTR-references/scripts/" 2>/dev/null || true
    elif [[ ! -d "$SCRIPT_DIR/HipSTR-references" ]]; then
        echo "Bundled scripts not found; downloading HipSTR references..."
        git clone https://github.com/HipSTR-Tool/HipSTR-references.git "$SCRIPT_DIR/HipSTR-references"
        chmod -R 755 "$SCRIPT_DIR/HipSTR-references/scripts/"
    else
...
setup_reference_genome() {
    ...
    if [[ -n "$REFERENCE_FILE" ]]; then
        ...
        if [[ ! -f "${GENOME}.fa" ]]; then
            ...
            # UCSC official download servers (primary + mirrors).
            # NOTE: Chinese open-source mirrors (tuna/ustc) do NOT host UCSC
            # genome FASTA data, so they are not usable here. Users behind a
            # firewall that blocks UCSC should instead download the FASTA
            # manually and use the --reference option.
            local mirrors=(
                "https://hgdownload.soe.ucsc.edu/goldenPath"
                "https://hgdownload.gi.ucsc.edu/goldenPath"
                "https://hgdownload-euro.soe.ucsc.edu/goldenPath"
            )
...
prepare_trf() {
    ...
    if [[ ! -f "trf409.legacylinux64" ]]; then
        ...
        chmod +x trf409.legacylinux64 2>/dev/null || true
    fi
