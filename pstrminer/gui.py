...
DANGER      = "#DC2626"
DANGER_HOV  = "#B91C1C"
NEUTRAL     = "#94A3B8"      # Stop button (idle / disabled)
NEUTRAL_ON  = "#64748B"      # Stop button, armed
NEUTRAL_HOV = "#475569"
SUCCESS     = "#15803D"
WARN        = "#B45309"
TEXT        = "#1E293B"
...
class StopButton(tk.Button):
    ...
    Rendered in a neutral slate rather than red: it sits next to the primary
    Run button and should not read as a destructive action. The idle/disabled
    shade is the lighter NEUTRAL; once a run is in progress the button arms
    itself to the darker NEUTRAL_ON so the two states are distinguishable.
    """

    def __init__(self, parent, command):
        super().__init__(parent, text="Stop", command=command, font=FONT_BTN,
                         bg=NEUTRAL, fg="white", activebackground=NEUTRAL_HOV,
                         activeforeground="white", relief="flat",
                         disabledforeground="white",
                         cursor="hand2", padx=18, pady=9, bd=0,
                         state="disabled")
        self.bind("<Enter>", lambda e: self.config(bg=NEUTRAL_HOV)
                  if str(self["state"]) != "disabled" else None)
        self.bind("<Leave>", lambda e: self.config(bg=self._rest())
                  if str(self["state"]) != "disabled" else None)

    def _rest(self):
        return NEUTRAL if str(self["state"]) == "disabled" else NEUTRAL_ON

    def configure(self, cnf=None, **kw):
        r = super().configure(cnf, **kw)
        if "state" in kw:
            super().configure(bg=self._rest())
        return r
...
class ScrollArea(tk.Frame):
    """A vertically scrollable container for the parameter cards.

    Add widgets to ``.body``. The scrollbar is only mapped when the content is
    taller than the visible area, so a short tab looks exactly as it did before
    this class existed. The inner frame is kept at the canvas width so cards
    never get clipped horizontally by the scrollbar.
    """
    ...
    def __init__(self, parent):
        ...
        self._canvas = tk.Canvas(self, bg=BG, highlightthickness=0, bd=0,
                                 takefocus=0)
        self._bar = tk.Scrollbar(self, orient="vertical",
        ...
        # Wheel support, bound only while the pointer is over this area so the
        # log console keeps its own scrolling.
...
class ConfigTab(tk.Frame):
    ...
    def _build(self):
        ...
        # Action row is pinned above the status bar; the cards in between get
        # whatever height is left and scroll if they need more.
        action = tk.Frame(left, bg=BG)
        action.pack(side="bottom", fill="x", pady=(2, 6))

        scroller = ScrollArea(left)
        scroller.pack(side="top", fill="both", expand=True)
        body = scroller.body

        # --- Reference genome (local first, default local) ----
        ref = Card(body, "Reference Genome Source")
        ...
        trf = Card(body, "TRF  (Tandem Repeats Finder)")
...
class PolyTab(tk.Frame):
    ...
    def _build(self):
        ...
        # whatever height is left and scroll if they need more.
        action = tk.Frame(left, bg=BG)
        action.pack(side="bottom", fill="x", pady=(GAP, 6))
...
HELP_CONTENT = [
    ...
    ("h1", "More"),
    ("link", "Repository:  github.com/Sunlab-forensicsysu/pSTRminer"),
...
class App(tk.Tk):
    ...
    def _build_footer(self):
        ...
        tk.Label(ft, text="github.com/Sunlab-forensicsysu/pSTRminer",
