...
# Integration of call-level VCF filtering and R script genotype conversion
# Usage: ./run_STR_filter_convert.sh -C <config_file> -I <input_VCF> -O <output_prefix> -F <T/F>

# Directory containing this script (and the bundled STR_analysis_pipeline.R).
# Using BASH_SOURCE makes script-location independent of the working directory.
...
INPUT_VCF=""
OUTPUT_PREFIX=""
FILTER_SCRIPT="$SCRIPT_DIR/pstr_filter_vcf.py"
...
usage() {
    ...
    echo "  --filter-script <path>   : Path to the VCF filter script"
    echo "                             (default: bundled pstr_filter_vcf.py)"
...
check_parameter_compatibility() {
    ...
    if [[ "$NEEDS_FILTER" == "T" ]] && [[ ! -f "$FILTER_SCRIPT" ]]; then
        print_error "VCF filter script not found: $FILTER_SCRIPT"
...
preprocess_vcf() {
    ...
    # The bundled filter reads plain and gzip-compressed VCFs directly and
    # does not need a tabix index, so no compression or indexing is required.
    VCF_GZ="$INPUT_VCF"

    if [[ "$NEEDS_FILTER" == "T" ]]; then
        if [[ "$INPUT_VCF" == *.gz ]]; then
            print_info "Input VCF is gzip-compressed; reading it directly"
        else
            print_info "Input VCF is uncompressed; reading it directly"
        fi
...
run_genotype_conversion() {
    ...
    # Check if R script exists (located alongside this script)
    if [[ ! -f "$SCRIPT_DIR/STR_analysis_pipeline.R" ]]; then
        print_error "R script not found: $SCRIPT_DIR/STR_analysis_pipeline.R"
        print_info "Please ensure STR_analysis_pipeline.R is in the same directory as this script"
        exit 1
    fi
    ...
    # Execute R script
    Rscript "$SCRIPT_DIR/STR_analysis_pipeline.R" "$VCF_TO_CONVERT" "$CONFIG_FILE"
...
show_workflow_summary() {
    ...
    if [[ "$NEEDS_FILTER" == "T" ]]; then
        ...
        print_info "  2. Preprocess VCF file"
        print_info "  3. Filter with the bundled pstr_filter_vcf.py"
...
# Main function
main() {
    # Parse arguments first: the summary below reports their values.
    parse_arguments "$@"
