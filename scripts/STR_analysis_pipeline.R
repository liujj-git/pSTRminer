...
Ae_value <- function(x) {
  # Effective number of alleles, Ae = 1 / sum(p_i^2), computed from the allele
  # frequencies directly. Note it is not derived from He: He above carries the
  # N/(N-1) unbiased correction, so 1 - He is not the homozygosity.
  #
  # A monomorphic locus returns 0, not 1. Strictly, a monomorphic locus has one
  # effective allele, but 0 is used here as the "not applicable" marker for
  # monomorphic loci, consistently with MP, PD, PE2, PE3 and PIC above, and with
  # the reference implementation used for the accompanying study. Downstream
  # summaries select polymorphic loci with Ae != 0, so this convention must be
  # preserved for the published results to be reproducible.
  if (length(x) == 1) {
    return(0)
  }
  return(1 / sum(x^2))
